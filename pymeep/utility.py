...
from glob import glob1, has_magic
import re
...
def parse_ctl_params(lines):
    """Parse the lines from a meep ctl file and return a dictionary with all 
    parsed parameters.

    Note: This is a very simple implementation using non-recursive regex. It 
    will not check or worry about matching parentheses in the expressions.

    """
    result = {}

    # regular expression pattern for the four possibilities to define or set a parameter in libctl:
    retest = re.compile(r'\((?:define|define-param|set!|set-param!) (?P<key>\S+) (?P<val>.+)\)')
    for line in lines:
        m = retest.search(line)
        if m is not None:
            # found a match!
            gd = m.groupdict()
            result[gd['key']] = gd['val']
    return result
...
def mpb_data(h5file, dataset, outputfile, workingdir, x_tiles=1, y_tiles=1):
    ...
    if has_magic(h5file):
        ...
        if len(names) == 0:
            ...
            log.warning('mpb_data: Warning: Globbing found multiple'
                ' matching filenames, but will only use first one: %s' % names[0])
    ...
    callstr = defaults.mpbdata_call % dict(
                ...
                tiles_x=x_tiles, tiles_y=y_tiles)
...
def h5topng_pwr(h5file, outputfile, epsfile, workingdir, sliceparam='-0z0'):
    ...
    if has_magic(h5file):
        ...
        if len(names) == 0:
            ...
            log.warning('h5topng: Warning: Globbing found multiple'
                ' matching filenames, but will only use first one: %s' % names[0])
        # only load the first one found:
        h5file = names[0]
    callstr = defaults.pwrh5topng_call % dict(
                h5_file=h5file,
                output_file=outputfile,
                eps_file=epsfile,
...
def h5topng_field_series(field_component, epsfile, workingdir, x_tiles=1, y_tiles=1, sliceparam='-0z0'):
    ...
    if len(h5files) == 0:
        log.warning("h5topng: Could not open file: %s" % field_component + '-*.h5')
        return 1
    ...
    dataset = sp.check_output(["h5ls", path.join(workingdir, h5files[0])]).decode('utf-8').split()[0]
    # Export real part:
    if dataset == 'hz.i': dataset = 'hz.r'
    for h5file in h5files:
        # call mpbdata for each file:
        error = mpb_data(h5file, dataset, 'temp_' + h5file, workingdir, x_tiles=x_tiles, y_tiles=y_tiles)
        if error:
            return 1
    # and now h5topng on all files in one call, so that we can use -R:
    callstr = defaults.fieldh5topng_call % dict(
                h5_file=' '.join(['temp_' + f for f in h5files]),
                eps_file=epsfile,
                sliceparam=sliceparam)
    log.info("running: %s (in working dir: %s)" % (callstr, workingdir))
    ...
    if not path.exists(dname):
        mkdir(dname)
    for h5file in h5files:
        rename(path.join(workingdir, 'temp_' + h5file.rstrip('h5') + 'png'), path.join(dname, h5file.rstrip('h5') + 'png'))
        remove(path.join(workingdir, 'temp_' + h5file))


def export_gds_elements_locations_and_radii(
        gds_filename, cellname, output_filename, layers=None, unit=None, shift_granularity=0, number_format='%.8f', delimiter='\t'):
    """Exports to a list of coordinates and radii, ready for importing by libctl.

    layers is a list of layers to be exported. If None, all layers will be exported.

    Returns bounding box of structure.
    """
    from pymeep.gdsreader import GDSReader
    reader = GDSReader(gds_filename, unit)
    data = reader.get_element_locations_and_radii(cellname, layers=layers, shift_granularity=shift_granularity)
    #np.savetxt(output_filename, data, fmt=number_format, delimiter=delimiter)
    # save especially for guile:
    lineformat = "(" + delimiter.join([number_format] * data.shape[1]) + ")\n"
    with open(output_filename, 'w') as f:
        for d in data:
            f.write(lineformat % tuple(d))
    return np.array(((data[:, 0].min(), data[:, 1].min()), (data[:, 0].max(), data[:, 1].max())))
